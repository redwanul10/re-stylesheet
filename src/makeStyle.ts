...
const init = (
  ...
  const styles =
    typeof styleOrFun === 'function' ? styleOrFun(options) : styleOrFun;
...
const makeStyle = <T extends (theme: breakpoint) => TestStylesType2>(
  styleOrFun: T
): ((
  props: any,
  size?: boolean | undefined
  ...
  const useStyle = (props: any, size: boolean | undefined) => {
    ...
    if (!cacheStyleConfig) {
      const config = init(styleOrFun, data, width, accessed);
    ...
    if (isPropsOrThemeOrDeviceTypeChanged) {
      fs.current = applyDynamicValues(styleConfig.current, props);
      prevProps.current = JSON.stringify(props);
      prevThemeId.current = data.theme.themeId;
      activeMediaQuery.current = device;
    }

    return { styles: fs.current, deviceType: device };
  };
...
// custom media query (*)
// reduce Dimension.onchange event call (*)
// optimize getDeviceType method (*)
// validate mediaquery arg in breakpoint method (*)
// create useMediaQuery Hooks (*)
// autosugges mediaquery in breakpoint method
